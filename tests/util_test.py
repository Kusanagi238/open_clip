...
class TestWrapper(torch.nn.Module):
    ...
    def __init__(self, model=None, model_name=None, output_dict=True) -> None:
        super().__init__()
        self.model = model
        self.output_dict = output_dict
        # Only configure model-specific attributes if a model is provided
        if model is not None:
            if type(model) in [open_clip.CLIP, open_clip.CustomTextCLIP]:
                self.model.output_dict = self.output_dict
        # Create the head only if a model_name is provided; otherwise leave it unset
    ...
    def forward(self, image, text):
        if self.model is None or self.head is None:
            raise RuntimeError(
                "TestWrapper requires 'model' and 'model_name' to be set before calling forward."
            )
